/*
 ...
 History
  ...
  1.1.0 02/10/2016:   Many bug fixes and updates 
  =======================================================
*/
#ifndef H_BUS_NODE_H
#define H_BUS_NODE_H
#define VERSION_HBUS_NODE_H "1.1.0"
...
class HNode : HBusMessageHandler, public HBusPinController, HBusDeviceController, HBusSensorController {
  ...
  public:
    ...
    void onPinChange(uint8_t index, char *name, uint32_t value);
...
//-----------------------------------------------------------
void HNode::onPinChange(uint8_t index, char *name, uint32_t value)
{
  ...
  if (pin->listener != 0) {
    ...
    _tx->pushLong(value);
#if defined(DEBUG) && DEBUG == 1 //DEBUG on Serial
   Serial << "onPinChange " << index << " - " << name << " = " << value << endl;
#endif
   if (value == 0) {
  ...
  //Send event to direct sketch code
  if (onPinChangeCallback != 0)
    onPinChangeCallback(index, name, value);
}
//-----------------------------------------------------------
...
uint8_t HNode::getDeviceStatus(char* name)
{
    ...
    _tx->clearWriteIndex();
    _tx->pushByte(dev->info.index);
    _tx->pushName(dev->info.name);
...
uint8_t HNode::addSensor(Sensor *sensor) 
{
  ...
  if (info.sensors == MAX_SENSORS)  
  {
    _bus->setError(MAX_SENSORS_REACHED);
    return 0xff;
  }
...
uint8_t HNode::addSensorListener(char* name, hb_address_t source, uint8_t port, uint8_t interval, uint16_t expire)
{
    for(uint8_t i = 0; i < info.sensors; i++)
    {
      ...
      if (strcmp(name, _sensor[i]->info.name) == 0 || strcmp(name, EMPTY_NAME) == 0) {
        ...
        _sensor[i]->info.interval = interval;
...
void HNode::feedSensorListeners()
{
 ...
 // Serial << "\ts[" << i << "] " << _sensor[i]->info.name << " => "
  ...
  // <<  endl;
      ...
      _tx->pushLong(value.time);
      _tx->pushFloat(value.value);
      _sensor[i]->count = _sensor[i]->info.interval;
...
uint8_t HNode::processCommand(hb_message_t message, uint8_t port) 
{
  ...
  Pin *new_pin;
  Wire *new_wire;
  hb_sensor_read_t svalue;
  uint32_t lvalue;
  uint8_t index;
  if (_status != NODE_ACTIVE && message.command != CMD_PING && message.command != CMD_START && message.command != CMD_RESET) return false;
...
//      Serial << "=> CMD_DELETE_PIN_LISTENER" << endl;
      ...
      break;
    case CMD_PUSH_PIN_EVENT:
      if (onPinChangeCallback != 0) {
        index = _rx->popByte();
        _rx->popName(_tmp_name); //Device name
        lvalue = _rx->popLong();
        onPinChangeCallback(index, _tmp_name, lvalue);
      }
      break;
    //---------------------------------------------------------------
    // Write commands
...
//  Serial << "=> CMD_ADD_SENSOR_LISTENER" << endl;
 ...
 //Serial << "=> CMD_DELETE_SENSOR_LISTENER" << endl;
    ...
    //  Serial << "=> CMD_PUSH_SENSOR_READ" << endl;
      ...
      _rx->popName(svalue.name);
      svalue.time = _rx->popLong();
      svalue.value= _rx->popFloat();
      //Call external handler
      if (onSensorReadCallback!= 0)
        processed = onSensorReadCallback(&svalue);
      break;
#endif                    
    default:
        processed = false;
