/*
  ...
  HBus - Bus controller serial test program

  This example uses 2 serial ports , one for HBus communications
  and the other for debug messages.
  If you want only test HBus you can use Arduinos with one port
  or use ethernet example
  Copyright (C) 2014 Vincenzo Mennella (see license.txt)

  History
    1.0.0 13/06/2014:   First code release
    1.0.1 25/08/2015:   Code review and documentation
  =======================================================
*/
...
HBusPort *port = new HBusSerialPort(&HBUS_SERIAL, 0);
...
#if defined(DEBUG) && DEBUG == 1 //DEBUG on SERIAL
  DEBUG_PORT << "HBus controller serial test program" << endl;
#endif
